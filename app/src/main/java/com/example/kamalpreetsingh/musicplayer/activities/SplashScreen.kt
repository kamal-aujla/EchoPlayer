...
class SplashScreen : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(! hasAllPermissions(this@SplashScreen,*permissionsString)){
            //this block will execute when permissions are not guaranted.
            //We need to ask for permissions
        ActivityCompat.requestPermissions(this@SplashScreen,permissionsString,131)
        }


            shift()//if all the permissions are guaranted
        ...
        when(requestCode){
            131->{
                if(grantResults.isNotEmpty() && grantResults[0]==PackageManager.PERMISSION_GRANTED
                    ...
                    Toast.makeText(this@SplashScreen,"Permissions not guaranted",Toast.LENGTH_SHORT).show()
                    this.finish()
                }
                return
            }
